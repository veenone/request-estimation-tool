...
pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'IS_STAGING', defaultValue: false, description: 'Tag and push the image as :staging instead of :latest (the :<IMAGE_TAG> tag is always pushed)')
        string(name: 'AGENT_LABEL', defaultValue: 'I2J62-0409', description: 'Jenkins agent label. Windows agent needs PuTTY (plink/pscp); Linux agent needs openssh-client + sshpass.')
